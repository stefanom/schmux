	...
	for {
		...
		case msg, ok := <-controlChan:
			...
			case "input":
				...
				// Filter out terminal response sequences (DA responses, cursor reports)
				// These should not be sent to tmux as they pollute the terminal
				if isTerminalResponse(msg.Data) {
					fmt.Printf("[websocket] filtered terminal response: %q\n", msg.Data)
					break
				}
...
// isTerminalResponse checks if data is a terminal response sequence that should be filtered.
// Terminal responses include Device Attributes (DA) responses and cursor position reports.
// These are automatically generated by terminal emulators and should not be sent to tmux.
func isTerminalResponse(data string) bool {
	// Check for CSI sequences ending with 'c' (DA response) or 'R' (cursor position)
	// Full CSI: ESC [ ... c or ESC [ ... R
	...
	// Check for partial responses like "0;276;0c" (numbers/semicolons ending in 'c')
	if len(data) >= 2 && data[len(data)-1] == 'c' {
